...
def adapt_domains(cr, model, old, new, adapter=None, skip_inherit=(), force_adapt=False, skip_jobrad_custom_contract_views=_SKIP_JOBRAD_VIEWS_DEFAULT):
    """
    ...
    if skip_jobrad_custom_contract_views:
        cr.execute("SELECT id, model, {} FROM ir_ui_view WHERE name NOT LIKE %s AND {} ~ %s".format(active_col, arch_db),
    ...
    for inh in for_each_inherit(cr, target_model, skip_inherit):
        adapt_domains(
            cr, inh.model, old, new, adapter, skip_inherit=skip_inherit, force_adapt=force_adapt,
